...
def download_cat(path,url, **kwargs): #need to catch cases where files are packed - what about csv? - conversion?
    if kwargs['cat'].upper() == 'LOTSS':
        ...
        download_lotss_catalogue(kwargs['msin'], ResultsFile=kwargs['lotss_output'], Radius=float(kwargs['lotss_radius']), AllFile=kwargs['lotss_output']+'_all', DoDownload=True)
    else:
